...
API_BASE = os.environ.get("SUNO_STUDIO_API_BASE") or "https://studio-api.prod.suno.com"
DEFAULT_USER_AGENT = (
    ...
    def list_library_cursor(
        ...
        *,
        ...
        try:
            ...
            # A 401 here used to abort immediately instead of falling
            # through to v2/legacy like every other status code already
            # does. v3 is the only feed endpoint that also checks the
            # `browser-token` header (a reverse-engineered, unsigned
            # anti-bot value -- see _browser_token()); if Suno tightens
            # that check, a perfectly valid bearer token can 401 on v3
            # while v2/legacy would still accept it. Falling through gives
            # that a real chance instead of failing the whole connection.
            v3_error = str(exc)
