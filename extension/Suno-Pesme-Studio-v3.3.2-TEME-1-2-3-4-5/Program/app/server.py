...
def _suno_auto_reopen_browser() -> bool:
    # Was default "0": the Suno connection lives entirely in a separate,
    # dedicated automation browser window (see cdp.py's ChromeConnector) --
    # not the user's everyday browser. A user closing that "extra" window
    # (a very natural reaction) silently killed the Suno connection with no
    # automatic recovery unless they had already found and enabled this
    # setting. Defaulting to on means the app tries to recover on its own
    # first; it can still be turned off in Settings.
    return DB.get_setting("suno_auto_reopen_browser", "1") == "1"
...
def _song_finder_candidates(upload_signature: dict[str, Any], songs: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], int]:
    ...
    for song in shortlist:
        ...
        try:
            ...
            # song_finder's own 4-6s thresholds (see song_finder.py) were
            # previously unreachable dead code: compare_signatures() defaults
            # to a 12s/46-score floor tuned for the full-video-completeness
            # feature, so any short Shorts clip was discarded before
            # classify_match() ever ran. Pass song_finder's real thresholds
            # through explicitly for this short-clip feature.
            analysis = compare_signatures(
                song_signature, upload_signature,
                min_match_seconds=song_finder.POSSIBLE_MIN_SECONDS,
                min_candidate_score=song_finder.POSSIBLE_MIN_SCORE,
            )
...
def song_finder_analyze(path: Path) -> dict[str, Any]:
    ...
    # _song_finder_candidates -- no source file/URL is touched here), so
    # filtering this list to _existing_audio_path() served no purpose except
    # to silently exclude every song that song_finder_index_task() had
    # fingerprinted directly from its remote Suno URL (common for large
    # libraries that were never bulk-downloaded): those songs cost real time
    # and bandwidth to index, then were never actually searchable. Any song
    # with a cached fingerprint for the current algorithm is a valid
    # candidate regardless of whether its audio source is still reachable.
    songs = DB.export_rows()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _send_file(self, path: Path, download_name: str | None = None, no_cache: bool = False) -> None:
        ...
        if range_header and range_header.startswith("bytes="):
            try:
                ...
                return
        length = end - start + 1
        # Open the file BEFORE committing any response headers. Opening
        # used to happen after send_response()/Content-Length was already
        # flushed to the socket -- if open() failed (e.g. a background
        # tagging/processing job briefly holds an exclusive Windows file
        # handle on this exact song), the response silently ended with 0
        # bytes despite promising `length` bytes, which the browser's
        # media pipeline reports as a network/decode error with no
        # indication of the real cause.
        try:
            fh_context = path.open("rb")
        except OSError:
            self.send_error(503, "Fajl je trenutno zauzet, pokušaj ponovo za koji trenutak.")
            return
        self.send_response(status)
    ...
    def _proxy_remote(self, url: str, download_name: str | None = None, refresh_url: Callable[[], str] | None = None) -> None:
        # Streaming (playback) used to trust `url` forever, unlike the
        # download path in suno_client.py which already refreshes the
        # Suno token/CDN link and retries on 401/403/429. A signed Suno CDN
        # URL or the cached session token can go stale while the app sits
        # open, so without this the player would get a 401/403 back from
        # Suno, turn it into a RuntimeError -> JSON 500 body, and the
        # <audio> element would just silently fail to play.
        attempts = 2 if refresh_url is not None else 1
        response = None
        for attempt in range(attempts):
            current_url = _validated_public_remote_url(url)
            headers = {
            ...
            with STATE_LOCK:
                token = SESSION_TOKEN
            if token and "studio-api.prod.suno.com" in current_url:
            ...
            try:
                ...
                if exc.code == 416:
                    ...
                    self.end_headers()
                    return
                can_retry = exc.code in (401, 403) and refresh_url is not None and attempt < attempts - 1
                refreshed = ""
                if can_retry:
                    try:
        ...
        try:
            ...
            if path == "/api/song/audio":
                ...
                if file_id:
                    ...
                    refresh_audio_url: Callable[[], str] | None = None
                else:
                    song = DB.get_song(song_id)
                    if not song:
                        raise RuntimeError("Pesma nije pronađena.")
                    ...
                    def refresh_audio_url(sid: str = song_id) -> str:
                        # Same recovery the download path already relies on
                        # (see _song_audio_source_for_match) -- called by
                        # _proxy_remote only when Suno actually returns
                        # 401/403 on a stream request, so a still-good URL
                        # never pays this extra API round-trip.
                        detail = get_client().get_clip(sid)
                        DB.upsert_song(detail)
                        refreshed_song = DB.get_song(sid) or {}
                        return str(refreshed_song.get("audio_url") or "")
                ...
                if source_path:
                    ...
                    self._proxy_remote(remote_url, filename, refresh_url=refresh_audio_url if SESSION_TOKEN else None)
                else:
