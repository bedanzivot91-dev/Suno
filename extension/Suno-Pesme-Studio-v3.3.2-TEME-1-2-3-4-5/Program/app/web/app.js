...
const THEMES = [
  { id:'broadcast-redline', name:'Broadcast Redline', colors:['#ef1018','#f2f2f2','#070707'], full:true, desc:'Kompletna radio/TV kontrolna konzola sa ON AIR panelom, redom emitovanja i VU metrima.' },
  { id:'street-mixtape', name:'Nedostaješ PUNOO', colors:['#2e5bd4','#ffd12f','#e73b34','#dedbd2'], full:true, desc:'Potpuno novi urbani raspored sa kasetama, posterom, brzim AI alatima i kasetofonskim plejerom.' },
  { id:'label-command', name:'Label Command', colors:['#d21f2b','#d9d6cc','#111214','#628f72'], full:true, desc:'Kompletna komandna tabla izdavačke kuće sa katalogom, performansama, planerom objava i distribucijom.' },
  { id:'vinyl-loft', name:'Vinyl Loft', colors:['#c88b43','#5b341f','#17120f','#e2c49b'], full:true, desc:'Topli vinilni studio sa policom albuma, omotima ploča, drvenim plejerom, VU metrima i gramofonom.' },
  { id:'signal-grid', name:'Signal Grid', colors:['#2e8fff','#141a1f','#f28a2a','#59e188'], full:true, desc:'Profesionalni rack interfejs sa tehničkom tabelom, talasnim prikazom, CPU/RAM/DISK modulima i hardverskim transportom.' },
];
...
const viewText = {
  home: ['Početna', 'Glavna kontrolna tabla aktivne teme i trenutne pesme.'],
...
function showView(name) {
  if (!viewText[name]) name = 'home'; state.activeView = name;
  ...
  qsa('.br-nav').forEach((el) => el.classList.toggle('active', el.dataset.brView === name));
  qsa('.sm-nav,[data-sm-view]').forEach((el) => el.classList.toggle('active', el.dataset.smView === name && el.classList.contains('sm-nav')));
  qsa('.lc-nav').forEach((el) => el.classList.toggle('active', el.dataset.lcView === name));
  qsa('.vl-nav').forEach((el) => el.classList.toggle('active', el.dataset.vlView === name));
  qsa('.sg-nav').forEach((el) => el.classList.toggle('active', el.dataset.sgView === name));
  document.body.classList.toggle('br-home-active', name === 'home');
  document.body.classList.toggle('sm-home-active', name === 'home');
  document.body.classList.toggle('lc-home-active', name === 'home');
  document.body.classList.toggle('vl-home-active', name === 'home');
  document.body.classList.toggle('sg-home-active', name === 'home');
  if ($('lcPageTitle')) $('lcPageTitle').textContent = (viewText[name]?.[0] || name).toUpperCase();
  if (name === 'home') { qsa('[data-br-queue-tab]').forEach((el) => el.classList.toggle('active', el.dataset.brQueueTab === 'queue')); }
  $('pageTitle').textContent = viewText[name][0]; $('pageSubtitle').textContent = viewText[name][1];
  if (name === 'home') { updateBroadcastRedlineChrome(); updateStreetMixtapeChrome(); updateLabelCommandChrome(); updateVinylLoftChrome(); updateSignalGridChrome(); }
...
function renderSongs() {
  ...
  }); updateSelectionUi(); updateBroadcastRedlineChrome(); updateStreetMixtapeChrome(); updateVinylLoftChrome(); updateSignalGridChrome();
}
...
async function loadSongs(resetPage = false) {
  try {
    ...
    renderSongs();
    updateBroadcastRedlineChrome(); updateStreetMixtapeChrome(); updateVinylLoftChrome(); updateSignalGridChrome();
...
async function openSong(id, showModal = true) {
  try {
    const data = await api(`/api/song?id=${encodeURIComponent(id)}`); const song = data.song; state.currentSong = song; updateBroadcastRedlineChrome(); updateStreetMixtapeChrome(); updateVinylLoftChrome(); updateSignalGridChrome();
...
async function playSong(song, fileId = 0, start = null, end = null) {
  ...
  state.currentSong = song; updateBroadcastRedlineChrome(); updateStreetMixtapeChrome(); updateVinylLoftChrome(); updateSignalGridChrome(); updateStreetMixtapeChrome();
  ...
  // file_sha256 changes whenever the audio bytes on disk actually change
  // (re-download, re-processing) -- appended so the player's "already
  // loaded this exact src, just seek" shortcut below can't keep serving a
  // stale/previously-failed version of the same song id after it's fixed.
  const cacheBust = fileId ? '' : String(song.file_sha256 || song.content_sha256 || song.downloaded_at || '');
  const src = fileId ? `/api/song/audio?file_id=${fileId}` : `/api/song/audio?id=${encodeURIComponent(song.id)}${cacheBust ? `&v=${encodeURIComponent(cacheBust)}` : ''}`;
...
function applyTheme(themeId, persist = true) {
  const valid = THEMES.some((t) => t.id === themeId) ? themeId : 'signal-grid';
  document.body.dataset.theme = valid;
  document.body.classList.toggle('broadcast-redline-app', valid === 'broadcast-redline');
  document.body.classList.toggle('street-mixtape-app', valid === 'street-mixtape');
  document.body.classList.toggle('label-command-app', valid === 'label-command');
  document.body.classList.toggle('vinyl-loft-app', valid === 'vinyl-loft');
  document.body.classList.toggle('signal-grid-app', valid === 'signal-grid');
  localStorage.setItem('suno-theme', valid);
  if ($('themeSelect')) $('themeSelect').value = valid;
  qsa('.theme-card').forEach((x) => x.classList.toggle('active', x.dataset.theme === valid));
  if (valid === 'broadcast-redline') updateBroadcastRedlineChrome();
  if (valid === 'street-mixtape') updateStreetMixtapeChrome();
  if (valid === 'label-command') updateLabelCommandChrome();
  if (valid === 'vinyl-loft') updateVinylLoftChrome();
  if (valid === 'signal-grid') updateSignalGridChrome();
  if (persist) api('/api/settings', { method:'POST', body:{theme:valid} }).catch(()=>{});
}
function renderThemes() {
  if (!$('themeGrid')) return;
  const active = document.body.dataset.theme || localStorage.getItem('suno-theme') || 'signal-grid';
  $('themeSelect').innerHTML = THEMES.map((t)=>`<option value="${t.id}">${escapeHtml(t.name)}</option>`).join('');
  $('themeGrid').innerHTML = THEMES.map((t)=>`<button class="theme-card" data-theme="${t.id}"><span class="theme-colors">${t.colors.map((c)=>`<i style="background:${c}"></i>`).join('')}</span><strong>${escapeHtml(t.name)}</strong>${t.desc?`<small>${escapeHtml(t.desc)}</small>`:''}</button>`).join('');
...
function resetThemeAndAppearance() {
  ...
  applyTheme('label-command');
  applyA11ySettings(loadA11ySettings());
  toast('Vraćena je Label Command tema i podrazumevana podešavanja izgleda.', 'success');
}
...
function bindEvents() {
  ...
  $('sgHomeConnectSunoBtn')?.addEventListener('click',()=>state.status?.connected?showView('import'):connectStart());
  $('sgHomeConnectYoutubeBtn')?.addEventListener('click',()=>state.status?.youtube_oauth?.connected?showView('tools'):connectYoutubeGoogle());
...
// ==========================================================================
// BROADCAST REDLINE — kompletan funkcionalni radio-studio raspored.
// Ovo nije promena palete: tema uvodi novu gornju navigaciju, red emitovanja,
// aktivnu pesmu, ON AIR/VU konzolu i zaseban transportni pult. Originalni
// elementi i ID-jevi ostaju netaknuti, pa postojeće funkcije nastavljaju da rade.
// ==========================================================================
let brVuFrame = 0;
let brAutoplay = true;

function brSongImage(song) {
  if (!song) return '';
  return song.local_cover ? `/media?path=${encodeURIComponent(song.local_cover)}` : (song.image_url || '');
}
function brArtist(song) { return song?.display_name || song?.artist || song?.source_group || 'Suno Pesme'; }
function brTotalDuration(rows) { return (rows || []).reduce((sum, song) => sum + Number(song?.duration || 0), 0); }
function brCurrentSong() { return state.currentSong || state.songs.find(songPlayable) || state.songs[0] || null; }
function brNextSong(current) {
  if (!state.songs.length) return null;
  const idx = current ? state.songs.findIndex((s) => String(s.id) === String(current.id)) : -1;
  return state.songs[(idx >= 0 ? idx + 1 : 1) % state.songs.length] || null;
}
function buildBroadcastWaveform() {
  const bars = $('brWaveformBars'); const playedBars = $('brWaveformPlayedBars');
  if (!bars || !playedBars || bars.childElementCount) return;
  const base = [18,31,46,70,52,38,64,88,47,29,73,58,82,42,25,67,94,61,44,77,55,98,63,34,72,90,49,79,40,66,93,57,28,74,86,45,62,81,36,59,91,43,76,54,30,69,84,51,96,65,41,80,60,89,48,68,33,75,92,53,71,39,61,87,46,73,37,95,58,66,31,78,90,47,70,35,60,83,44,68,88,55,32,75,63,94,50,67,40,82,56,72,29,77,64];
  const pattern = [...base, ...base.map((h, i) => Math.max(16, Math.min(98, h + ((i * 13) % 19) - 9)))];
  const markup = pattern.map((h, i) => `<i style="height:${h}%;animation-delay:${(i%11)*-0.045}s"></i>`).join('');
  bars.innerHTML = markup; playedBars.innerHTML = markup;
}
function renderBroadcastQueue() {
  const list = $('brQueueList'); if (!list) return;
  const query = ($('brQueueSearch')?.value || '').trim().toLocaleLowerCase('sr');
  const rows = state.songs.filter((s) => !query || `${s.title||''} ${brArtist(s)}`.toLocaleLowerCase('sr').includes(query)).slice(0, 12);
  const current = brCurrentSong();
  list.innerHTML = rows.length ? rows.map((song, index) => {
    const active = current && String(current.id) === String(song.id);
    const image = brSongImage(song);
    return `<article class="br-queue-row ${active ? 'active' : ''}" data-song-id="${escapeHtml(song.id)}"><span class="br-live-slot">${active ? '<em>ON AIR</em>' : '<i>⠿</i>'}</span><b class="br-position">${index + 1}</b><div class="br-qcover">${image ? `<img src="${escapeHtml(image)}" alt="">` : '<span>♫</span>'}</div><div class="br-qtext"><strong>${escapeHtml(song.title || 'Bez naslova')}</strong><span>${escapeHtml(brArtist(song))}</span></div><div class="br-qtime"><strong>${formatDuration(song.duration)}</strong><span>${escapeHtml(song.bpm || '—')} BPM</span></div><button type="button">⋮</button></article>`;
  }).join('') : '<div class="br-queue-empty">Nema pesama u redu emitovanja.</div>';
  qsa('.br-queue-row', list).forEach((row) => row.addEventListener('click', () => {
    const song = state.songs.find((s) => String(s.id) === String(row.dataset.songId));
    if (song) playSong(song);
  }));
  const total = brTotalDuration(rows);
  if ($('brQueueSummary')) $('brQueueSummary').textContent = `${rows.length} pesama · ${formatDuration(total)}`;
  if ($('brQueueTotal')) $('brQueueTotal').textContent = formatDuration(total);
}
function updateBroadcastRedlineChrome() {
  if (!$('brActiveTitle')) return;
  buildBroadcastWaveform(); renderBroadcastQueue();
  const song = brCurrentSong(); const next = brNextSong(song); const audio = $('audioPlayer');
  const setImage = (id, src, placeholderId='') => { const el=$(id); if(!el)return; if(src){el.src=src;el.hidden=false;el.style.display='block';if(placeholderId&&$(placeholderId))$(placeholderId).style.display='none';}else{el.removeAttribute('src');el.hidden=true;el.style.display='none';if(placeholderId&&$(placeholderId))$(placeholderId).style.display='grid';} };
  if (song) {
    setImage('brActiveCover', brSongImage(song), 'brCoverPlaceholder');
    $('brActiveTitle').textContent = song.title || 'Bez naslova';
    $('brActiveArtist').textContent = brArtist(song);
    $('brActiveDuration').textContent = formatDuration(song.duration);
    $('brActiveBpm').textContent = `${song.bpm || '—'} BPM`;
    $('brActiveKey').textContent = song.key_signature || song.key || '—';
    $('brActivePublisher').textContent = song.publisher || 'Suno Pesme Studio';
    $('brActiveGenre').textContent = song.genre || (song.tags || '').split(',')[0] || '—';
    $('brActiveYear').textContent = song.year || String(song.created_at || '').slice(0,4) || '—';
    $('brActiveAdded').textContent = formatDate(song.created_at);
    $('brTotal').textContent = `${formatDuration(song.duration)}.0`;
  } else {
    setImage('brActiveCover','', 'brCoverPlaceholder');
    $('brActiveTitle').textContent='Nijedna pesma nije izabrana'; $('brActiveArtist').textContent='Suno Pesme Studio';
  }
  if (next) {
    setImage('brNextCover', brSongImage(next)); $('brNextTitle').textContent=next.title||'Bez naslova'; $('brNextArtist').textContent=brArtist(next); $('brNextMeta').textContent=`◷ ${formatDuration(next.duration)} · ◴ ${next.bpm||'—'} BPM`;
  } else { setImage('brNextCover',''); $('brNextTitle').textContent='Nema sledeće pesme'; $('brNextArtist').textContent='—'; $('brNextMeta').textContent='◷ 00:00 · ◴ — BPM'; }
  if (audio) {
    const duration = Number(audio.duration || song?.duration || 0); const current = Number(audio.currentTime || 0); const pct = duration ? Math.max(0,Math.min(100,current/duration*100)) : 0;
    if($('brWaveformPlayed')) $('brWaveformPlayed').style.width=`${pct}%`;
    if($('brPlayhead')) $('brPlayhead').style.left=`${pct}%`;
    if($('brElapsed')) $('brElapsed').textContent=`${formatDuration(current)}.${Math.floor((current%1)*10)}`;
    const playGlyph = audio.paused ? '▶' : 'Ⅱ';
    ['brMainPlayBtn','brBottomPlay'].forEach((id)=>{if($(id))$(id).textContent=playGlyph;});
    document.body.classList.toggle('br-playing', !audio.paused);
  }
}
function brPlayCurrent() { const a=$('audioPlayer'); if(a?.src){a.paused?a.play().catch(()=>{}):a.pause();return;} const song=brCurrentSong(); if(song)playSong(song); else toast('Biblioteka je prazna.','warning'); }
function brPlayNext() { const current=brCurrentSong(); const next=brNextSong(current); if(next)playSong(next); }
function brPlayPrev() { const current=brCurrentSong(); if(!state.songs.length)return; const idx=current?state.songs.findIndex((s)=>String(s.id)===String(current.id)):0; const prev=state.songs[(idx-1+state.songs.length)%state.songs.length]; if(prev)playSong(prev); }
function brUpdateClock() {
  const now=new Date(); if($('brClock'))$('brClock').textContent=now.toLocaleTimeString('sr-RS',{hour12:false});
  if($('brDate'))$('brDate').textContent=now.toLocaleDateString('sr-Latn-RS',{weekday:'long',day:'2-digit',month:'long',year:'numeric'});
}
function brAnimateVu() {
  const a=$('audioPlayer'); const playing=a&&!a.paused; const t=performance.now()/180;
  const l=playing?Math.round(38+34*Math.abs(Math.sin(t))+18*Math.random()):11;
  const r=playing?Math.round(35+36*Math.abs(Math.sin(t+0.7))+18*Math.random()):9;
  if($('brMeterL'))$('brMeterL').style.height=`${Math.min(98,l)}%`; if($('brMeterR'))$('brMeterR').style.height=`${Math.min(98,r)}%`;
  if($('brMeterLValue'))$('brMeterLValue').textContent=`-${Math.max(1,(30-l/3)).toFixed(1)} dB`; if($('brMeterRValue'))$('brMeterRValue').textContent=`-${Math.max(1,(30-r/3)).toFixed(1)} dB`;
  brVuFrame=requestAnimationFrame(brAnimateVu);
}
function setupBroadcastRedline() {
  buildBroadcastWaveform(); brUpdateClock(); setInterval(brUpdateClock,1000); if(!brVuFrame)brAnimateVu();
  qsa('[data-br-view]').forEach((b)=>b.addEventListener('click',()=>{showView(b.dataset.brView);document.body.classList.remove('br-more-open');$('brMoreNav')?.setAttribute('aria-hidden','true');}));
  qsa('[data-br-queue-tab]').forEach((b)=>b.addEventListener('click',()=>{qsa('[data-br-queue-tab]').forEach(x=>x.classList.remove('active'));b.classList.add('active'); if(b.dataset.brQueueTab==='queue')showView('home'); if(b.dataset.brQueueTab==='library')showView('library'); if(b.dataset.brQueueTab==='playlists')showView('folders');}));
  $('brQueueSearch')?.addEventListener('input',renderBroadcastQueue);
  $('brAddQueueBtn')?.addEventListener('click',()=>showView('library'));
  $('brHamburger')?.addEventListener('click',()=>{const open=!document.body.classList.contains('br-more-open');document.body.classList.toggle('br-more-open',open);$('brMoreNav')?.setAttribute('aria-hidden',open?'false':'true');});
  $('brMoreClose')?.addEventListener('click',()=>{document.body.classList.remove('br-more-open');$('brMoreNav')?.setAttribute('aria-hidden','true');});
  document.addEventListener('keydown',(e)=>{if(e.key==='Escape'){document.body.classList.remove('br-more-open');$('brMoreNav')?.setAttribute('aria-hidden','true');}});
  ['brMainPlayBtn','brBottomPlay'].forEach((id)=>$(id)?.addEventListener('click',brPlayCurrent));
  ['brPrevBtn','brBottomPrev'].forEach((id)=>$(id)?.addEventListener('click',brPlayPrev));
  ['brNextBtn','brBottomNext'].forEach((id)=>$(id)?.addEventListener('click',brPlayNext));
  $('brBottomStop')?.addEventListener('click',()=>{const a=$('audioPlayer');if(a){a.pause();a.currentTime=0;}updateBroadcastRedlineChrome();});
  $('brCueBtn')?.addEventListener('click',()=>{const a=$('audioPlayer');if(a){a.pause();a.currentTime=0;}updateBroadcastRedlineChrome();});
  $('brHeadphonesBtn')?.addEventListener('click',(e)=>e.currentTarget.classList.toggle('active'));
  $('brBottomShuffle')?.addEventListener('click',(e)=>e.currentTarget.classList.toggle('active'));
  qsa('.br-ab-group button').forEach((b)=>b.addEventListener('click',()=>{qsa('.br-ab-group button').forEach(x=>x.classList.remove('active'));b.classList.add('active');}));
  $('brLoopBtn')?.addEventListener('click',()=>{state.repeat=!state.repeat;$('brLoopBtn').classList.toggle('active',state.repeat);$('brBottomRepeat')?.classList.toggle('active',state.repeat);});
  $('brBottomRepeat')?.addEventListener('click',()=>{$('brLoopBtn')?.click();});
  $('brAutoplayToggle')?.addEventListener('click',()=>{brAutoplay=!brAutoplay;$('brAutoplayToggle').classList.toggle('active',brAutoplay);});
  $('brVolumeSlider')?.addEventListener('input',(e)=>{const v=Number(e.target.value);if($('audioPlayer'))$('audioPlayer').volume=v/100;$('brVolumeValue').textContent=`${v}%`;});
  $('brTempoSlider')?.addEventListener('input',(e)=>{const bpm=Number(e.target.value);$('brTempoValue').textContent=bpm.toFixed(2);if($('audioPlayer'))$('audioPlayer').playbackRate=Math.max(.5,Math.min(2,bpm/128));});
  $('brPitchSlider')?.addEventListener('input',(e)=>{$('brPitchValue').textContent=`${Number(e.target.value).toFixed(2)}%`;});
  $('brCrossfadeSlider')?.addEventListener('input',(e)=>{$('brCrossfadeValue').textContent=`${e.target.value} sec`;});
  $('brResetTransport')?.addEventListener('click',()=>{$('brPitchSlider').value=0;$('brTempoSlider').value=128;$('brCrossfadeSlider').value=5;$('brPitchValue').textContent='0.00%';$('brTempoValue').textContent='128.00';$('brCrossfadeValue').textContent='5 sec';if($('audioPlayer'))$('audioPlayer').playbackRate=1;});
  $('brStartJingle')?.addEventListener('click',()=>toast('Jingle je spreman za emitovanje.','success'));
  $('brRecordBtn')?.addEventListener('click',(e)=>{e.currentTarget.classList.toggle('active');toast(e.currentTarget.classList.contains('active')?'Snimanje je uključeno.':'Snimanje je zaustavljeno.','info');});
  qsa('[data-br-action="metadata"]').forEach((b)=>b.addEventListener('click',async()=>{const s=brCurrentSong();if(!s)return toast('Izaberi pesmu.','warning');await openSong(s.id);switchModalTab('tags');}));
  qsa('[data-br-action="jingle"]').forEach((b)=>b.addEventListener('click',()=>$('brStartJingle')?.click()));
  $('brWaveformWrap')?.addEventListener('click',(e)=>{const a=$('audioPlayer');if(!a||!a.duration)return;const r=e.currentTarget.getBoundingClientRect();a.currentTime=Math.max(0,Math.min(a.duration,(e.clientX-r.left)/r.width*a.duration));});
  const audio=$('audioPlayer'); if(audio){['play','pause','timeupdate','loadedmetadata'].forEach((ev)=>audio.addEventListener(ev,updateBroadcastRedlineChrome));audio.addEventListener('ended',()=>{updateBroadcastRedlineChrome();if(brAutoplay)brPlayNext();});}
  showView('home'); updateBroadcastRedlineChrome();
}



// ========================================================================
// STREET MIXTAPE — Theme 2. Real independent application shell.
// ========================================================================
let smSetupDone = false;
let smWaveReady = false;

function smArtist(song) { return song?.display_name || song?.artist || song?.source_group || 'Suno Pesme'; }
function smSongImage(song) { return song?.image_url || song?.cover_url || song?.local_cover_url || ''; }
function smSetImage(id, src, placeholderId='') { const el=$(id); if(!el)return; if(src){el.src=src;el.hidden=false;el.style.display='block';if(placeholderId&&$(placeholderId))$(placeholderId).style.display='none';}else{el.removeAttribute('src');el.hidden=true;el.style.display='none';if(placeholderId&&$(placeholderId))$(placeholderId).style.display='grid';} }
function smCurrentSong() { return state.currentSong || state.songs.find(songPlayable) || state.songs[0] || null; }
function smNextSong(current) {
  if (!state.songs.length) return null;
  const index = current ? state.songs.findIndex((s) => String(s.id) === String(current.id)) : -1;
  return state.songs[(index + 1 + state.songs.length) % state.songs.length] || state.songs[0];
}
function buildStreetWaveform() {
  if (smWaveReady || !$('smWaveBars') || !$('smWavePlayedBars')) return;
  const seed = [14,22,35,48,27,62,44,74,38,55,30,68,46,82,57,33,71,52,90,42,61,28,77,49,66,36,85,53,72,31,64,47];
  const html = Array.from({length: 128}, (_, i) => `<i style="height:${seed[(i * 7 + i % 5) % seed.length]}%"></i>`).join('');
  $('smWaveBars').innerHTML = html; $('smWavePlayedBars').innerHTML = html; smWaveReady = true;
}
function renderStreetRecent() {
  const list = $('smRecentList'); if (!list) return;
  const rows = state.songs.slice(0, 5); const current = smCurrentSong();
  if (!rows.length) { list.innerHTML = '<div class="sm-empty-tape"><strong>NEMA PESAMA</strong><span>Poveži Suno ili uvezi lokalni folder.</span><button data-sm-view="import" type="button">UVEZI PESME</button></div>'; return; }
  list.innerHTML = rows.map((song, index) => {
    const active = current && String(current.id) === String(song.id); const color = ['blue','red','blue','yellow','white'][index % 5];
    return `<article class="sm-track-row ${active ? 'active' : ''}" data-song-id="${escapeHtml(song.id)}"><div class="sm-row-cassette ${color}"><span>${escapeHtml((song.title || 'MIXTAPE').slice(0, 16).toUpperCase())}</span><i></i><em></em></div><div class="sm-track-main"><strong>${escapeHtml(song.title || 'Bez naslova')}</strong><div><span>${escapeHtml(song.genre || (song.tags || '').split(',')[0] || 'MIX')}</span><span>BPM ${escapeHtml(song.bpm || '—')}</span><span>KEY ${escapeHtml(song.key_signature || song.key || '—')}</span></div></div><time><b>${formatDate(song.created_at).slice(0,5)}</b><span>${formatDuration(song.duration)}</span></time><button class="sm-row-play" type="button" title="Pusti">▶</button><button type="button" title="Opcije">•••</button></article>`;
  }).join('');
  list.querySelectorAll('.sm-track-row').forEach((row) => row.addEventListener('click', (e) => {
    const song = state.songs.find((s) => String(s.id) === String(row.dataset.songId));
    if (!song) return; state.currentSong = song; updateStreetMixtapeChrome(); updateBroadcastRedlineChrome();
    if (e.target.closest('.sm-row-play')) playSong(song);
  }));
}
function updateStreetMixtapeChrome() {
  if (!$('smPlayerTitle')) return;
  buildStreetWaveform(); renderStreetRecent();
  const song = smCurrentSong(); const audio = $('audioPlayer');
  const totalSongs = Number(state.libraryTotal || state.totalFiltered || state.songs.length || 0);
  const seconds = state.songs.reduce((sum, row) => sum + Number(row.duration || 0), 0);
  if ($('smTotalSongs')) $('smTotalSongs').textContent = totalSongs.toLocaleString('sr-RS');
  if ($('smGeneratedSongs')) $('smGeneratedSongs').textContent = Math.max(0, Math.round(totalSongs * .25)).toLocaleString('sr-RS');
  if ($('smHoursMusic')) $('smHoursMusic').textContent = `${Math.max(0, Math.round(seconds / 3600))}h`;
  if ($('smExportedSongs')) $('smExportedSongs').textContent = Math.max(0, Math.round(totalSongs * .08)).toLocaleString('sr-RS');
  if ($('smTotalStreams')) $('smTotalStreams').textContent = Math.max(0, totalSongs * 1720).toLocaleString('sr-RS');
  if (song) {
    smSetImage('smPlayerCover', smSongImage(song), 'smPlayerCoverPlaceholder');
    $('smPlayerTitle').textContent = song.title || 'Bez naslova'; $('smPlayerArtist').textContent = smArtist(song);
    $('smPlayerMeta').textContent = `${escapeHtml(song.genre || 'MIX')} · ${song.bpm || '—'} BPM · ${song.key_signature || song.key || '—'}`;
    $('smTotal').textContent = formatDuration(song.duration || audio?.duration || 0);
  } else {
    smSetImage('smPlayerCover', '', 'smPlayerCoverPlaceholder'); $('smPlayerTitle').textContent = 'Nijedna pesma nije puštena'; $('smPlayerArtist').textContent = 'Izaberi pesmu'; $('smPlayerMeta').textContent = '— · — BPM · —'; $('smTotal').textContent = '00:00';
  }
  const duration = Number(audio?.duration || song?.duration || 0); const current = Number(audio?.currentTime || 0); const pct = duration ? Math.max(0, Math.min(100, current / duration * 100)) : 0;
  if ($('smWavePlayed')) $('smWavePlayed').style.width = `${pct}%`; if ($('smWaveCursor')) $('smWaveCursor').style.left = `${pct}%`; if ($('smElapsed')) $('smElapsed').textContent = formatDuration(current);
  if ($('smPlayBtn')) $('smPlayBtn').querySelector('span').textContent = audio && !audio.paused ? 'Ⅱ' : '▶';
  const knob = $('smVolumeKnob'); const slider = $('smVolumeSlider'); if (knob && slider) knob.style.setProperty('--sm-volume-angle', `${-135 + Number(slider.value) * 2.7}deg`);
}
function smPlayCurrent() { const audio = $('audioPlayer'); if (audio?.src) { audio.paused ? audio.play().catch(()=>{}) : audio.pause(); return; } const song = smCurrentSong(); if (song) playSong(song); else toast('Biblioteka je prazna.', 'warning'); }
function smPlayNext() { const next = smNextSong(smCurrentSong()); if (next) playSong(next); }
function smPlayPrev() { if (!state.songs.length) return; const current = smCurrentSong(); const i = current ? state.songs.findIndex((s) => String(s.id) === String(current.id)) : 0; const prev = state.songs[(i - 1 + state.songs.length) % state.songs.length]; if (prev) playSong(prev); }
function setupStreetMixtape() {
  if (smSetupDone) return; smSetupDone = true; buildStreetWaveform();
  qsa('[data-sm-view]').forEach((button) => button.addEventListener('click', () => { showView(button.dataset.smView); document.body.classList.remove('sm-pages-open'); $('smPagesDrawer')?.setAttribute('aria-hidden','true'); }));
  $('smAllPagesToggle')?.addEventListener('click', () => { const open = !document.body.classList.contains('sm-pages-open'); document.body.classList.toggle('sm-pages-open', open); $('smPagesDrawer')?.setAttribute('aria-hidden', open ? 'false' : 'true'); });
  $('smPagesClose')?.addEventListener('click', () => { document.body.classList.remove('sm-pages-open'); $('smPagesDrawer')?.setAttribute('aria-hidden','true'); });
  document.addEventListener('keydown', (e) => { if (e.key === 'Escape') { document.body.classList.remove('sm-pages-open'); $('smPagesDrawer')?.setAttribute('aria-hidden','true'); } });
  $('smPlayBtn')?.addEventListener('click', smPlayCurrent); $('smPrevBtn')?.addEventListener('click', smPlayPrev); $('smNextBtn')?.addEventListener('click', smPlayNext);
  $('smStopBtn')?.addEventListener('click', () => { const audio = $('audioPlayer'); if (audio) { audio.pause(); audio.currentTime = 0; } updateStreetMixtapeChrome(); });
  $('smRecordBtn')?.addEventListener('click', (e) => { e.currentTarget.classList.toggle('active'); toast(e.currentTarget.classList.contains('active') ? 'REC je uključen.' : 'REC je isključen.', 'info'); });
  $('smVolumeSlider')?.addEventListener('input', (e) => { const value = Number(e.target.value); if ($('audioPlayer')) $('audioPlayer').volume = value / 100; updateStreetMixtapeChrome(); });
  $('smWaveBars')?.parentElement?.addEventListener('click', (e) => { const audio = $('audioPlayer'); if (!audio || !audio.duration) return; const rect = e.currentTarget.getBoundingClientRect(); audio.currentTime = Math.max(0, Math.min(audio.duration, (e.clientX - rect.left) / rect.width * audio.duration)); });
  const audio = $('audioPlayer'); if (audio) { ['play','pause','timeupdate','loadedmetadata'].forEach((ev) => audio.addEventListener(ev, updateStreetMixtapeChrome)); audio.addEventListener('ended', () => { updateStreetMixtapeChrome(); smPlayNext(); }); }
  updateStreetMixtapeChrome();
}



// ========================================================================
// LABEL COMMAND — Theme 3. Independent record-label application shell.
// ========================================================================
let lcSetupDone=false;
function lcArtist(song){return song?.display_name||song?.artist||song?.source_group||'Suno Pesme';}
function lcImage(song){return song?.image_url||song?.cover_url||song?.local_cover_url||'';}
function lcSetImage(id,src,placeholderId=''){const el=$(id);if(!el)return;if(src){el.src=src;el.hidden=false;el.style.display='block';if(placeholderId&&$(placeholderId))$(placeholderId).style.display='none';}else{el.removeAttribute('src');el.hidden=true;el.style.display='none';if(placeholderId&&$(placeholderId))$(placeholderId).style.display='grid';}}
function lcCurrent(){return state.currentSong||state.songs.find(songPlayable)||state.songs[0]||null;}
function lcNext(current,dir=1){if(!state.songs.length)return null;const index=current?state.songs.findIndex((s)=>String(s.id)===String(current.id)):-1;return state.songs[(index+dir+state.songs.length)%state.songs.length]||state.songs[0];}
function lcStatus(index){return ['ZAVRŠENA','MASTERED','U OBRADI','MIXING','OBJAVLJENA','OBJAVLJENA'][index%6];}
function lcStatusClass(status){return status.toLowerCase().replaceAll(' ','-').replaceAll('ž','z');}
function renderLabelRows(){const target=$('lcSongRows');if(!target)return;const rows=state.songs.slice(0,6);if(!rows.length){target.innerHTML='<div class="lc-empty-catalog">Biblioteka je prazna — poveži Suno nalog ili uvezi pesme.</div>';return;} const current=lcCurrent();target.innerHTML=rows.map((song,index)=>{const active=current&&String(current.id)===String(song.id);const status=lcStatus(index);return `<article class="lc-song-row ${active?'active':''}" data-song-id="${escapeHtml(song.id)}"><span>${active?'▥':index+1}</span><span class="lc-row-title"><button type="button">▶</button><i>${(song.title||'S').slice(0,1).toUpperCase()}</i><b>${escapeHtml(song.title||'Bez naslova')}</b></span><span>${escapeHtml(lcArtist(song))}</span><span>${escapeHtml(song.album||`${song.title||'Singl'} - Single`)}</span><span>${escapeHtml(song.genre||'Pop')}</span><span>${formatDuration(song.duration)}</span><span><em class="lc-row-status ${lcStatusClass(status)}">${status}</em></span><span>${formatDate(song.created_at).slice(0,10)}</span><span>⋮</span></article>`;}).join('');target.querySelectorAll('.lc-song-row').forEach((row)=>row.addEventListener('click',(e)=>{const song=state.songs.find((s)=>String(s.id)===String(row.dataset.songId));if(!song)return;state.currentSong=song;updateLabelCommandChrome();updateBroadcastRedlineChrome();updateStreetMixtapeChrome();if(e.target.closest('button'))playSong(song);}));}
function renderLabelPlanner(){const target=$('lcPlannerList');if(!target)return;const rows=state.songs.slice(0,4);target.innerHTML=(rows.length?rows:[null,null,null,null]).map((song,index)=>`<article><i>${song?(song.title||'S').slice(0,1).toUpperCase():'S'}</i><div><b>${escapeHtml(song?.title||['Noćas','Tvoje usne','Leto bez tebe','Grad od sna'][index])}</b><small>${escapeHtml(song?lcArtist(song):'Suno Pesme')}</small></div><time>${String(30+index*7).padStart(2,'0')}.0${5+(index%2)}.2026.<small>PET&nbsp;&nbsp;00:00</small></time><button>⋮</button></article>`).join('');}
function updateLabelCommandChrome(){if(!$('lcHeroTitle'))return;const song=lcCurrent(),audio=$('audioPlayer');const total=Number(state.libraryTotal||state.totalFiltered||state.songs.length||0);const streams=Math.max(0,total*1002);if($('lcStreamsKpi'))$('lcStreamsKpi').textContent=streams.toLocaleString('sr-RS');if($('lcListenersKpi'))$('lcListenersKpi').textContent=Math.round(streams*.25).toLocaleString('sr-RS');if($('lcYoutubeKpi'))$('lcYoutubeKpi').textContent=Math.round(streams*.344).toLocaleString('sr-RS');if($('lcPlaylistKpi'))$('lcPlaylistKpi').textContent=Math.round(total*1.7).toLocaleString('sr-RS');renderLabelRows();renderLabelPlanner();if(song){lcSetImage('lcHeroCover',lcImage(song),'lcHeroCoverPlaceholder');lcSetImage('lcPlayerCover',lcImage(song),'lcPlayerCoverPlaceholder');$('lcHeroTitle').textContent=song.title||'Bez naslova';$('lcHeroArtist').textContent=lcArtist(song);$('lcHeroGenre').textContent=song.genre||'Pop';$('lcHeroSubgenre').textContent=(song.tags||'Synth Pop').split(',')[0];$('lcHeroKey').textContent=song.key_signature||song.key||'—';$('lcHeroDuration').textContent=formatDuration(song.duration);$('lcHeroTempo').textContent=`${song.bpm||'—'} BPM`;$('lcCatalogNumber').textContent=`SPS 2026-${String(Math.abs(Number(song.id)||1)).slice(-3).padStart(3,'0')}`;$('lcHeroDescription').textContent=`${song.title||'Ova pesma'} je izabrana za pregled u Label Command sistemu. Ovde se prate produkcija, metapodaci, distribucija, analitika i plan objave.`;$('lcPlayerTitle').textContent=song.title||'Bez naslova';$('lcPlayerArtist').textContent=lcArtist(song);$('lcTotal').textContent=formatDuration(song.duration||audio?.duration||0);}else{lcSetImage('lcHeroCover','','lcHeroCoverPlaceholder');lcSetImage('lcPlayerCover','','lcPlayerCoverPlaceholder');$('lcHeroTitle').textContent='Nijedna pesma nije izabrana';$('lcHeroArtist').textContent='Suno Pesme';$('lcPlayerTitle').textContent='Nijedna pesma nije puštena';$('lcPlayerArtist').textContent='Suno Pesme';$('lcTotal').textContent='00:00';}const duration=Number(audio?.duration||song?.duration||0),current=Number(audio?.currentTime||0),pct=duration?Math.max(0,Math.min(100,current/duration*100)):0;if($('lcElapsed'))$('lcElapsed').textContent=formatDuration(current);if($('lcProgress'))$('lcProgress').style.setProperty('--lc-progress',`${pct}%`);if($('lcPlayBtn'))$('lcPlayBtn').textContent=audio&&!audio.paused?'Ⅱ':'▶';}
function lcPlayCurrent(){const audio=$('audioPlayer');if(audio?.src){audio.paused?audio.play().catch(()=>{}):audio.pause();return;}const song=lcCurrent();if(song)playSong(song);else toast('Biblioteka je prazna.','warning');}
function setupLabelCommand(){if(lcSetupDone)return;lcSetupDone=true;qsa('[data-lc-view]').forEach((button)=>button.addEventListener('click',()=>{showView(button.dataset.lcView);document.body.classList.remove('lc-pages-open');$('lcPagesDrawer')?.setAttribute('aria-hidden','true');}));$('lcAllPagesToggle')?.addEventListener('click',()=>{document.body.classList.toggle('lc-pages-open');$('lcPagesDrawer')?.setAttribute('aria-hidden',document.body.classList.contains('lc-pages-open')?'false':'true');});$('lcPagesClose')?.addEventListener('click',()=>{document.body.classList.remove('lc-pages-open');$('lcPagesDrawer')?.setAttribute('aria-hidden','true');});$('lcPlayHero')?.addEventListener('click',lcPlayCurrent);$('lcPlayBtn')?.addEventListener('click',lcPlayCurrent);$('lcPrevBtn')?.addEventListener('click',()=>{const song=lcNext(lcCurrent(),-1);if(song)playSong(song);});$('lcNextBtn')?.addEventListener('click',()=>{const song=lcNext(lcCurrent(),1);if(song)playSong(song);});$('lcVolumeSlider')?.addEventListener('input',(e)=>{const audio=$('audioPlayer');if(audio)audio.volume=Number(e.target.value)/100;});$('lcProgress')?.addEventListener('click',(e)=>{const audio=$('audioPlayer');if(!audio?.duration)return;const r=e.currentTarget.getBoundingClientRect();audio.currentTime=Math.max(0,Math.min(1,(e.clientX-r.left)/r.width))*audio.duration;});$('lcGlobalSearch')?.addEventListener('input',(e)=>{const target=$('searchInput');if(target){target.removeAttribute('readonly');target.value=e.target.value;target.dispatchEvent(new Event('input',{bubbles:true}));}if(e.target.value.trim())showView('library');});const audio=$('audioPlayer');['timeupdate','play','pause','loadedmetadata','ended'].forEach((evt)=>audio?.addEventListener(evt,updateLabelCommandChrome));setInterval(()=>{if(document.body.dataset.theme==='label-command')updateLabelCommandChrome();},1200);updateLabelCommandChrome();}


// ========================================================================
// VINYL LOFT — Theme 4. Full warm record-room shell and turntable player.
// ========================================================================
let vlSetupDone = false;
function vlArtist(song){ return song?.display_name || song?.artist || song?.source_group || 'Suno Pesme'; }
function vlImage(song){ return song?.image_url || song?.cover_url || song?.local_cover_url || ''; }
function vlCurrent(){ return state.currentSong || state.songs.find(songPlayable) || state.songs[0] || null; }
function vlSongColor(index){ return ['rust','blue','violet','sepia','olive'][index % 5]; }
function vlSetImage(id, src, placeholderId=''){
  const el=$(id); if(!el) return;
  if(src){ el.src=src; el.hidden=false; el.style.display='block'; if(placeholderId&&$(placeholderId)) $(placeholderId).style.display='none'; }
  else { el.removeAttribute('src'); el.hidden=true; el.style.display='none'; if(placeholderId&&$(placeholderId)) $(placeholderId).style.display='grid'; }
}
function vlPlayCurrent(){ const audio=$('audioPlayer'); if(audio?.src){ audio.paused?audio.play().catch(()=>{}):audio.pause(); return; } const song=vlCurrent(); if(song) playSong(song); else toast('Biblioteka je prazna.','warning'); }
function vlStep(dir){ if(!state.songs.length) return; const current=vlCurrent(); let index=current?state.songs.findIndex(s=>String(s.id)===String(current.id)):-1; index=(index+dir+state.songs.length)%state.songs.length; const song=state.songs[index]; state.currentSong=song; playSong(song); }
function renderVinylRecent(){
  const target=$('vlRecentCards'); if(!target) return; const rows=state.songs.slice(0,5);
  if(!rows.length){ target.innerHTML='<div class="vl-empty">Biblioteka je prazna — poveži Suno nalog ili uvezi pesme.</div>'; return; }
  const current=vlCurrent();
  target.innerHTML=rows.map((song,index)=>{ const active=current&&String(current.id)===String(song.id); const src=vlImage(song); return `<article class="vl-album-card ${active?'active':''}" data-song-id="${escapeHtml(song.id)}"><div class="vl-disc"></div><div class="vl-album-cover ${vlSongColor(index)}">${src?`<img src="${escapeHtml(src)}" alt="">`:`<strong>${escapeHtml((song.title||'PESMA').toUpperCase())}</strong><small>SUNO PESME</small>`}</div><div class="vl-card-copy"><b>${escapeHtml(song.title||'Bez naslova')}</b><span>${escapeHtml(vlArtist(song))}</span><time>${formatDuration(song.duration)}</time></div><button type="button">⋮</button></article>`; }).join('');
  target.querySelectorAll('.vl-album-card').forEach(card=>card.addEventListener('click',e=>{ const song=state.songs.find(s=>String(s.id)===String(card.dataset.songId)); if(!song) return; state.currentSong=song; updateVinylLoftChrome(); if(!e.target.closest('.vl-card-copy button')) playSong(song); }));
}
function renderVinylRows(){
  const target=$('vlSongRows'); if(!target) return; const rows=state.songs.slice(0,7); const current=vlCurrent();
  target.innerHTML=rows.length?rows.map((song,index)=>`<article class="vl-song-row ${current&&String(current.id)===String(song.id)?'active':''}" data-song-id="${escapeHtml(song.id)}"><span>${current&&String(current.id)===String(song.id)?'▥':index+1}</span><span class="vl-row-title"><i class="${vlSongColor(index)}">${(song.title||'S').slice(0,1).toUpperCase()}</i><b>${escapeHtml(song.title||'Bez naslova')}</b></span><span>${escapeHtml(vlArtist(song))}</span><span>${escapeHtml(song.album||song.title||'Singl')}</span><span>${escapeHtml(song.genre||'Pop')}</span><span>${formatDuration(song.duration)}</span><span>${formatDate(song.created_at).slice(0,10)}</span><button type="button">${index===0?'♥':'♡'}</button><button type="button">⋮</button></article>`).join(''):'<div class="vl-empty">Nema pesama za prikaz.</div>';
  target.querySelectorAll('.vl-song-row').forEach(row=>row.addEventListener('click',()=>{ const song=state.songs.find(s=>String(s.id)===String(row.dataset.songId)); if(song){ state.currentSong=song; playSong(song); updateVinylLoftChrome(); }}));
}
function buildVinylWave(){
  ['vlMiniWaveBars','vlMiniWavePlayedBars'].forEach(id=>{ const el=$(id); if(el&&!el.children.length) el.innerHTML=Array.from({length:86},(_,i)=>`<i style="height:${18+((i*17)%43)}%"></i>`).join(''); });
}
function updateVinylLoftChrome(){
  if(!$('vlPlayerTitle')) return; buildVinylWave(); renderVinylRecent(); renderVinylRows();
  const total=Number(state.libraryTotal||state.totalFiltered||state.songs.length||0), duration=state.songs.reduce((sum,s)=>sum+Number(s.duration||0),0), song=vlCurrent(), audio=$('audioPlayer');
  if($('vlTotalSongs')) $('vlTotalSongs').textContent=total.toLocaleString('sr-RS');
  if($('vlTotalAlbums')) $('vlTotalAlbums').textContent=Math.max(1,Math.round(total/14)).toLocaleString('sr-RS');
  if($('vlTotalStreams')) $('vlTotalStreams').textContent=Math.max(0,total*47).toLocaleString('sr-RS');
  if($('vlTotalDuration')) $('vlTotalDuration').textContent=duration>=86400?`${Math.floor(duration/86400)}d ${Math.floor((duration%86400)/3600)}h ${Math.floor((duration%3600)/60)}m`:formatHours(duration);
  if(song){ vlSetImage('vlPlayerCover',vlImage(song),'vlPlayerCoverPlaceholder'); $('vlPlayerTitle').textContent=song.title||'Bez naslova'; $('vlPlayerArtist').textContent=vlArtist(song); $('vlTotal').textContent=formatDuration(song.duration||audio?.duration||0); }
  else { vlSetImage('vlPlayerCover','', 'vlPlayerCoverPlaceholder'); $('vlPlayerTitle').textContent='Nijedna pesma nije puštena'; $('vlPlayerArtist').textContent='Suno Pesme'; $('vlTotal').textContent='00:00'; }
  const d=Number(audio?.duration||song?.duration||0), c=Number(audio?.currentTime||0), pct=d?Math.max(0,Math.min(100,c/d*100)):0;
  if($('vlElapsed')) $('vlElapsed').textContent=formatDuration(c); if($('vlMiniWavePlayed')) $('vlMiniWavePlayed').style.width=`${pct}%`; if($('vlMiniWaveCursor')) $('vlMiniWaveCursor').style.left=`${pct}%`; if($('vlSeek')) $('vlSeek').value=String(Math.round(pct*10)); if($('vlPlayBtn')) $('vlPlayBtn').textContent=audio&&!audio.paused?'Ⅱ':'▶';
}
function setupVinylLoft(){
  if(vlSetupDone) return; vlSetupDone=true;
  qsa('[data-vl-view]').forEach(button=>button.addEventListener('click',()=>{ showView(button.dataset.vlView); document.body.classList.remove('vl-pages-open'); $('vlPagesDrawer')?.setAttribute('aria-hidden','true'); }));
  $('vlAllPagesToggle')?.addEventListener('click',()=>{ document.body.classList.toggle('vl-pages-open'); $('vlPagesDrawer')?.setAttribute('aria-hidden',document.body.classList.contains('vl-pages-open')?'false':'true'); });
  $('vlPagesClose')?.addEventListener('click',()=>{ document.body.classList.remove('vl-pages-open'); $('vlPagesDrawer')?.setAttribute('aria-hidden','true'); });
  $('vlPlayBtn')?.addEventListener('click',vlPlayCurrent); $('vlPrevBtn')?.addEventListener('click',()=>vlStep(-1)); $('vlNextBtn')?.addEventListener('click',()=>vlStep(1));
  $('vlVolumeSlider')?.addEventListener('input',e=>{ const audio=$('audioPlayer'); if(audio) audio.volume=Number(e.target.value)/100; });
  $('vlSeek')?.addEventListener('input',e=>{ const audio=$('audioPlayer'); if(audio?.duration) audio.currentTime=Number(e.target.value)/1000*audio.duration; });
  $('vlGlobalSearch')?.addEventListener('input',e=>{ const input=$('searchInput'); if(input){ input.removeAttribute('readonly'); input.value=e.target.value; input.dispatchEvent(new Event('input',{bubbles:true})); } if(e.target.value.trim()) showView('library'); });
  const audio=$('audioPlayer'); ['play','pause','timeupdate','loadedmetadata','ended'].forEach(evt=>audio?.addEventListener(evt,updateVinylLoftChrome));
  setInterval(()=>{ if(document.body.dataset.theme==='vinyl-loft') updateVinylLoftChrome(); },1500); updateVinylLoftChrome();
}

// ========================================================================
// SIGNAL GRID — Theme 5. Full hardware rack shell and technical dashboard.
// ========================================================================
let sgSetupDone=false;
function sgArtist(song){ return song?.display_name || song?.artist || song?.source_group || 'Suno Pesme'; }
function sgImage(song){ return song?.image_url || song?.cover_url || song?.local_cover_url || ''; }
function sgCurrent(){ return state.currentSong || state.songs.find(songPlayable) || state.songs[0] || null; }
function sgSetImage(id,src,placeholderId=''){ const el=$(id); if(!el)return; if(src){el.src=src;el.hidden=false;el.style.display='block';if(placeholderId&&$(placeholderId))$(placeholderId).style.display='none';}else{el.removeAttribute('src');el.hidden=true;el.style.display='none';if(placeholderId&&$(placeholderId))$(placeholderId).style.display='grid';} }
function buildSignalWave(){ ['sgMainWaveBars','sgMainWavePlayedBars'].forEach(id=>{const el=$(id);if(el&&!el.children.length)el.innerHTML=Array.from({length:175},(_,i)=>`<i style="height:${16+((i*29+i*i*7)%82)}%"></i>`).join('');}); ['sgBottomWaveBars','sgBottomWavePlayedBars'].forEach(id=>{const el=$(id);if(el&&!el.children.length)el.innerHTML=Array.from({length:110},(_,i)=>`<i style="height:${12+((i*23)%77)}%"></i>`).join('');}); }
function renderSignalRows(){
  const target=$('sgSongRows'); if(!target)return; const rows=state.songs.slice(0,12), current=sgCurrent();
  target.innerHTML=rows.length?rows.map((song,index)=>`<article class="sg-tech-row ${current&&String(current.id)===String(song.id)?'active':''}" data-song-id="${escapeHtml(song.id)}"><span>${index+1}</span><button type="button">${current&&String(current.id)===String(song.id)?'◀':'▶'}</button><span>${escapeHtml(song.title||'Bez naslova')}</span><span>${escapeHtml(sgArtist(song))}</span><span>${escapeHtml(song.album||song.title||'Singl')}</span><span>${escapeHtml(song.genre||'Pop')}</span><span>${formatDuration(song.duration)}</span><span>${song.bpm||'—'}</span><span>${escapeHtml(song.key_signature||song.key||'—')}</span><span>${formatDate(song.created_at).slice(0,10)}</span><button type="button">⋮</button></article>`).join(''):'<div class="sg-empty">NEMA PESAMA U SIGNALNOJ TABELI</div>';
  target.querySelectorAll('.sg-tech-row').forEach(row=>row.addEventListener('click',()=>{ const song=state.songs.find(s=>String(s.id)===String(row.dataset.songId)); if(song){state.currentSong=song;playSong(song);updateSignalGridChrome();} }));
}
function sgPlayCurrent(){ const audio=$('audioPlayer'); if(audio?.src){ audio.paused?audio.play().catch(()=>{}):audio.pause(); return; } const song=sgCurrent(); if(song) playSong(song); else toast('Biblioteka je prazna.','warning'); }
function sgStep(dir){ if(!state.songs.length)return; const current=sgCurrent(); let index=current?state.songs.findIndex(s=>String(s.id)===String(current.id)):-1; index=(index+dir+state.songs.length)%state.songs.length; const song=state.songs[index]; state.currentSong=song; playSong(song); }
function updateSignalClock(){ const now=new Date(); if($('sgClock'))$('sgClock').textContent=now.toLocaleTimeString('sr-RS',{hour12:false}); if($('sgDate'))$('sgDate').textContent=now.toLocaleDateString('sr-RS'); }
function updateSignalGridChrome(){
  if(!$('sgPlayerTitle'))return; buildSignalWave(); renderSignalRows(); updateSignalClock();
  if($('sgHomeSunoDot'))$('sgHomeSunoDot').classList.toggle('on',Boolean(state.status?.connected));
  if($('sgHomeYoutubeDot'))$('sgHomeYoutubeDot').classList.toggle('on',Boolean(state.status?.youtube_oauth?.connected));
  if($('sgHomeConnectSunoBtn'))$('sgHomeConnectSunoBtn').lastChild.textContent=state.status?.connected?' Suno povezan':' Poveži Suno nalog';
  if($('sgHomeConnectYoutubeBtn'))$('sgHomeConnectYoutubeBtn').lastChild.textContent=state.status?.youtube_oauth?.connected?' YouTube povezan':' Poveži YouTube nalog';
  const songs=state.songs, total=Number(state.libraryTotal||state.totalFiltered||songs.length||0), duration=songs.reduce((sum,s)=>sum+Number(s.duration||0),0), song=sgCurrent(), audio=$('audioPlayer');
  const artists=new Set(songs.map(s=>sgArtist(s)).filter(Boolean)), genres=new Set(songs.map(s=>s.genre).filter(Boolean)), albums=new Set(songs.map(s=>s.album).filter(Boolean));
  if($('sgTotalSongs'))$('sgTotalSongs').textContent=total.toLocaleString('sr-RS'); if($('sgTotalDuration'))$('sgTotalDuration').textContent=formatDuration(duration); if($('sgTotalArtists'))$('sgTotalArtists').textContent=artists.size.toLocaleString('sr-RS'); if($('sgTotalGenres'))$('sgTotalGenres').textContent=genres.size.toLocaleString('sr-RS'); if($('sgTotalAlbums'))$('sgTotalAlbums').textContent=Math.max(albums.size,Math.round(total/14)).toLocaleString('sr-RS'); if($('sgAddedToday'))$('sgAddedToday').textContent=Math.min(total,Math.max(0,songs.filter(s=>String(s.created_at||'').slice(0,10)===new Date().toISOString().slice(0,10)).length)).toLocaleString('sr-RS'); if($('sgTableSummary'))$('sgTableSummary').textContent=`UKUPNO ${total.toLocaleString('sr-RS')} PESAMA`;
  const cpu=18+(Date.now()/900%16), ram=38+(Date.now()/1400%11), disk=12; if($('sgCpuBar'))$('sgCpuBar').style.width=`${cpu}%`;if($('sgCpuValue'))$('sgCpuValue').textContent=`${Math.round(cpu)}%`;if($('sgRamBar'))$('sgRamBar').style.width=`${ram}%`;if($('sgRamValue'))$('sgRamValue').textContent=`${Math.round(ram)}%`;if($('sgDiskBar'))$('sgDiskBar').style.width=`${disk}%`;if($('sgDiskValue'))$('sgDiskValue').textContent=`${disk}%`;
  if(song){ sgSetImage('sgPlayerCover',sgImage(song),'sgPlayerCoverPlaceholder'); $('sgPlayerTitle').textContent=song.title||'Bez naslova'; $('sgPlayerArtist').textContent=sgArtist(song); $('sgPlayerBpm').textContent=song.bpm||'—'; $('sgPlayerKey').textContent=song.key_signature||song.key||'—'; $('sgTotal').textContent=formatDuration(song.duration||audio?.duration||0); }
  else { sgSetImage('sgPlayerCover','', 'sgPlayerCoverPlaceholder'); $('sgPlayerTitle').textContent='Nijedna pesma nije puštena'; $('sgPlayerArtist').textContent='Suno Pesme'; $('sgPlayerBpm').textContent='—'; $('sgPlayerKey').textContent='—'; $('sgTotal').textContent='00:00'; }
  const d=Number(audio?.duration||song?.duration||0), c=Number(audio?.currentTime||0), pct=d?Math.max(0,Math.min(100,c/d*100)):0; if($('sgElapsed'))$('sgElapsed').textContent=formatDuration(c); if($('sgMainWavePlayed'))$('sgMainWavePlayed').style.width=`${pct}%`; if($('sgMainWaveCursor'))$('sgMainWaveCursor').style.left=`${pct}%`; if($('sgBottomWavePlayed'))$('sgBottomWavePlayed').style.width=`${pct}%`; if($('sgBottomWaveCursor'))$('sgBottomWaveCursor').style.left=`${pct}%`; if($('sgPlayBtn'))$('sgPlayBtn').classList.toggle('active',!!audio&&!audio.paused); if($('sgPauseBtn'))$('sgPauseBtn').classList.toggle('active',!!audio&&audio.paused);
  const phase=Date.now()/280; const l=35+Math.sin(phase)*16, r=42+Math.cos(phase*1.09)*14; ['sgHomeMeterL','sgOutputL'].forEach(id=>{if($(id))$(id).style.height=`${Math.max(8,l)}%`;}); ['sgHomeMeterR','sgOutputR'].forEach(id=>{if($(id))$(id).style.height=`${Math.max(8,r)}%`;});
}
function setupSignalGrid(){
  if(sgSetupDone)return; sgSetupDone=true;
  qsa('[data-sg-view]').forEach(button=>button.addEventListener('click',()=>{showView(button.dataset.sgView);document.body.classList.remove('sg-pages-open');$('sgPagesDrawer')?.setAttribute('aria-hidden','true');}));
  $('sgAllPagesToggle')?.addEventListener('click',()=>{document.body.classList.toggle('sg-pages-open');$('sgPagesDrawer')?.setAttribute('aria-hidden',document.body.classList.contains('sg-pages-open')?'false':'true');}); $('sgPagesClose')?.addEventListener('click',()=>{document.body.classList.remove('sg-pages-open');$('sgPagesDrawer')?.setAttribute('aria-hidden','true');});
  $('sgPlayBtn')?.addEventListener('click',sgPlayCurrent); $('sgPauseBtn')?.addEventListener('click',()=>$('audioPlayer')?.pause()); $('sgStopBtn')?.addEventListener('click',()=>{const a=$('audioPlayer');if(a){a.pause();a.currentTime=0;}}); $('sgPrevBtn')?.addEventListener('click',()=>sgStep(-1)); $('sgNextBtn')?.addEventListener('click',()=>sgStep(1));
  $('sgVolumeSlider')?.addEventListener('input',e=>{const a=$('audioPlayer');if(a)a.volume=Number(e.target.value)/100;});
  $('sgTableSearch')?.addEventListener('input',e=>{const input=$('searchInput');if(input){input.removeAttribute('readonly');input.value=e.target.value;input.dispatchEvent(new Event('input',{bubbles:true}));}});
  $('sgMainWaveBars')?.parentElement?.addEventListener('click',e=>{const a=$('audioPlayer');if(!a?.duration)return;const r=e.currentTarget.getBoundingClientRect();a.currentTime=Math.max(0,Math.min(1,(e.clientX-r.left)/r.width))*a.duration;});
  const audio=$('audioPlayer'); ['play','pause','timeupdate','loadedmetadata','ended'].forEach(evt=>audio?.addEventListener(evt,updateSignalGridChrome)); setInterval(()=>{if(document.body.dataset.theme==='signal-grid')updateSignalGridChrome();},900); updateSignalGridChrome();
}


async function init() { const themeBuildKey='suno-theme-build-theme5-signal-grid'; if(!localStorage.getItem(themeBuildKey)){localStorage.setItem('suno-theme','signal-grid');localStorage.setItem(themeBuildKey,'1');} applyTheme(localStorage.getItem('suno-theme') || 'signal-grid', false); setupBroadcastRedline(); setupStreetMixtape(); setupLabelCommand(); setupVinylLoft(); setupSignalGrid(); applyA11ySettings(loadA11ySettings()); bindEvents(); protectLibrarySearchFromAutofill(); renderThemes(); await loadSecurityStatus(); if(state.security?.locked)return; await Promise.all([loadStatus(), loadCollections(), loadStats()]); await loadSongs(true); loadAudioToolsStatus(); loadAdvancedStatus(); loadMusicRecognitionStatus(); loadMusicRecognitionHistory(); loadSongFinderStatus(); autoRefreshSongFinderIndex(); loadWatchedFolders(); checkProgramUpdate(); setInterval(()=>{if(!state.security?.locked)loadStatus();},3000); }
