...
from audio_tools import ensure_fpcalc, ffmpeg_path, fpcalc_path, probe_audio
...
ALGORITHM_VERSION = "sps-spectral-v4-fpcalc"
FRAME_SECONDS = 0.5
SAMPLE_RATE = 2000
...
FPCALC_MAX_LENGTH_SECONDS = 1800  # fpcalc defaults to only the first 120s otherwise.


def _extract_chromaprint(ffmpeg: str, source: str, tempo: float = 1.0) -> list[int]:
    """Real Chromaprint fingerprint via the official `fpcalc` CLI.

    FFmpeg's own `-f chromaprint` muxer was tried first (see the old
    implementation this replaces) but the bundled "essentials" FFmpeg build
    does not actually include the chromaprint muxer -- confirmed by running
    it directly, it always failed with "Requested output format
    'chromaprint' is not known" -- so this function silently returned []
    for every file, and every match silently fell back to the much weaker
    dependency-free spectral comparator below. fpcalc decodes audio itself
    (it has its own bundled FFmpeg libs), so for the common tempo==1.0 case
    it can run directly against the original source file/URL. For a
    tempo-shifted variant, a real ffmpeg atempo pass has to render an
    intermediate file first -- fpcalc has no live atempo filter of its own.
    """
    fpcalc = fpcalc_path()
    if not fpcalc:
        try:
    ...
    try:
        if tempo != 1.0:
            handle, temp_name = tempfile.mkstemp(suffix=".wav", prefix="sps-fpcalc-")
            os.close(handle)
            temp_wav = Path(temp_name)
            render_args = [
                ffmpeg, "-y", "-hide_banner", "-loglevel", "error", "-i", source, "-vn",
                "-af", f"atempo={tempo:.4f}", "-ac", "1", "-ar", "44100", str(temp_wav),
            ]
            rendered = subprocess.run(
                render_args, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=180,
                creationflags=_creationflags(),
            )
            if rendered.returncode != 0 or not temp_wav.exists() or temp_wav.stat().st_size < 1024:
                return []
            fpcalc_input = str(temp_wav)
        else:
            fpcalc_input = source
        args = [fpcalc, "-raw", "-json", "-length", str(FPCALC_MAX_LENGTH_SECONDS), fpcalc_input]
        result = subprocess.run(
            args, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=180,
            text=True, encoding="utf-8", errors="replace",
            creationflags=_creationflags(),
        )
        ...
        values = payload.get("fingerprint")
...
def _chromaprint_candidates(source: dict[str, Any], video: dict[str, Any], min_seconds: float = 12.0) -> list[dict[str, Any]]:
    src = [int(x) for x in (source.get("chromaprint") or [])]
    vid = [int(x) for x in (video.get("chromaprint") or [])]
    if len(src) < 20 or len(vid) < 20:
        return []
    ...
    min_frames = max(16, int(min_seconds / max(src_step, 0.05)))
...
def extract_signature(
    ...
    raw = b"".join(chunks)
    # Was 8s -- higher than song_finder.py's own POSSIBLE_MIN_SECONDS (4.0),
    # which meant a short Shorts clip could get rejected here before ever
    # reaching compare_signatures()'s (now also lowered) duration check.
    if len(raw) < SAMPLE_RATE * 2 * 3:
...
def compare_signatures(
    source: dict[str, Any],
    video: dict[str, Any],
    min_match_seconds: float = 12.0,
    min_candidate_score: float = 46.0,
) -> dict[str, Any]:
    ...
    `min_match_seconds`/`min_candidate_score` default to the original
    thresholds tuned for the full YouTube-video-completeness feature. The
    short-clip "Pronalazač mojih pesama" feature (song_finder.py) passes its
    own, lower thresholds (4-6s) here explicitly -- previously this function
    unconditionally discarded anything under 12 continuous seconds BEFORE
    song_finder.classify_match() ever saw it, which made song_finder's own
    4s/6s constants unreachable dead code for any short Shorts clip.
    """
    ...
    min_frames = max(6, int(min_match_seconds / interval))
    raw_candidates: list[dict[str, Any]] = _chromaprint_candidates(source, video, min_seconds=min_match_seconds)
    ...
    reliable = [x for x in selected if float(x.get("audio_score") or 0) >= min_candidate_score and float(x.get("matched_seconds") or 0) >= min_match_seconds]
    if not reliable:
        reliable = selected[:1]
    ...
    if audio_score < min_candidate_score or covered_seconds < min_match_seconds:
