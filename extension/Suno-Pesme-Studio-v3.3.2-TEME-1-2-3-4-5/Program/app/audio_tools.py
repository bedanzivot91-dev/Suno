...
# The bundled FFmpeg "essentials" build above does NOT include the
# chromaprint muxer (verified directly: `ffmpeg -f lavfi ... -f chromaprint`
# reports "Requested output format 'chromaprint' is not known"), so audio
# recognition silently fell back to a much weaker dependency-free comparator
# for every single file. fpcalc is Chromaprint's own official CLI (the same
# tool AcoustID/MusicBrainz Picard use) and is pinned to a specific
# version+SHA-256 -- unlike ffmpeg/yt-dlp there is no official checksum
# sidecar file published alongside the release, so the hash below was
# cross-checked against the GitHub Releases API's own asset "digest" field.
FPCALC_DIR = ROOT / "tools" / "fpcalc"
FPCALC_VERSION = "1.6.1"
FPCALC_ASSET = f"chromaprint-fpcalc-{FPCALC_VERSION}-windows-x86_64.zip"
FPCALC_URL = f"https://github.com/acoustid/chromaprint/releases/download/v{FPCALC_VERSION}/{FPCALC_ASSET}"
...
def ensure_fpcalc(progress: Callable[[str, int], None] | None = None) -> str:
    existing = fpcalc_path()
    if existing:
        return existing
    if os.name != "nt":
        raise RuntimeError("fpcalc nije pronađen. Instaliraj chromaprint/fpcalc kroz sistemski paket menadžer.")

    FPCALC_DIR.mkdir(parents=True, exist_ok=True)
    archive = FPCALC_DIR / f"{FPCALC_ASSET}.part"
    if progress:
        progress("Preuzimam Chromaprint (fpcalc) audio-otisak alat...", 0)

    def dl_progress(done: int, total: int) -> None:
        percent = int(done * 100 / total) if total else 0
        if progress:
            progress(f"Preuzimam fpcalc: {percent}%", percent)

    try:
        _download(FPCALC_URL, archive, dl_progress)
        actual = _sha256(archive).lower()
        if actual != FPCALC_SHA256:
            raise RuntimeError("Kontrolni zbir fpcalc paketa nije ispravan. Preuzimanje je obrisano.")
        extract_dir = FPCALC_DIR / "extract"
        if extract_dir.exists():
            shutil.rmtree(extract_dir, ignore_errors=True)
        extract_dir.mkdir(parents=True, exist_ok=True)
        with zipfile.ZipFile(archive) as zf:
            bad = zf.testzip()
            if bad:
                raise RuntimeError(f"fpcalc ZIP je oštećen: {bad}")
            zf.extractall(extract_dir)
        found = next(extract_dir.rglob("fpcalc.exe"), None)
        if not found:
            raise RuntimeError("fpcalc.exe nije pronađen u preuzetom paketu.")
        shutil.copy2(found, FPCALC_DIR / "fpcalc.exe")
        shutil.rmtree(extract_dir, ignore_errors=True)
        archive.unlink(missing_ok=True)
        if progress:
            progress("fpcalc je spreman.", 100)
    except Exception:
        archive.unlink(missing_ok=True)
        raise
    ...
    result = fpcalc_path()
    if not result:
        raise RuntimeError("fpcalc je preuzet, ali program ne može da ga pokrene.")
    return result
