...
func box(text string, flags uintptr) uintptr { u:=syscall.NewLazyDLL("user32.dll"); p:=u.NewProc("MessageBoxW"); t,_:=syscall.UTF16PtrFromString(text); c,_:=syscall.UTF16PtrFromString(appName+" "+appVersion); r,_,_:=p.Call(0,uintptr(unsafe.Pointer(t)),uintptr(unsafe.Pointer(c)),flags); return r }
