...
func main(){
    ...
    rootUninstaller:=filepath.Join(sourceRoot,"DEINSTALIRAJ_PROGRAM.exe"); if st,er:=os.Stat(rootUninstaller);er==nil { if er=copyFile(rootUninstaller,filepath.Join(temp,"Deinstaliraj Suno Pesme Studio.exe"),st.Mode());er!=nil{box("Kopiranje deinstalera nije uspelo: "+er.Error(),0x10);return} } else { box("DEINSTALIRAJ_PROGRAM.exe nije pronađen pored installera.",0x10);return }
