...
function Download([string]$Url, [string]$Dest) {
    ...
    if (!(Test-Path $Dest) -or ((Get-Item $Dest).Length -le 0)) { throw "Preuzimanje nije uspelo: $Url" }
}
function Assert-Sha256([string]$Path, [string]$Expected) {
    $actual = (Get-FileHash $Path -Algorithm SHA256).Hash.ToLowerInvariant()
    if ($actual -ne $Expected.ToLowerInvariant()) { throw "SHA256 nije ispravan za $Path`nOcekivan: $Expected`nDobijen: $actual" }
}
...
if (Test-Path $Parts) {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Ne mogu da raspakujem V1 source archive.' }
...
# ---------------------------------------------------------------------------
# SELF-CONTAINED PYTHON 3.13 RUNTIME + exact runtime packages used by V1.
# ---------------------------------------------------------------------------
$PyVersion = '3.13.14'
...
if (!$Pth) { throw 'Python _pth nije pronađen.' }
...
if ($LASTEXITCODE -ne 0) { throw 'get-pip nije uspeo.' }
...
if ($LASTEXITCODE -ne 0) { throw 'Instalacija Python paketa nije uspela.' }

# ---------------------------------------------------------------------------
# SELF-CONTAINED FFmpeg + ffprobe.
# ---------------------------------------------------------------------------
...
$shaText = Get-Content $FFmpegSha -Raw
$shaMatch = [regex]::Match($shaText, '(?i)\b[a-f0-9]{64}\b')
if (!$shaMatch.Success) { throw 'FFmpeg SHA256 fajl nema validan hash.' }
...
if (!$ffmpeg -or !$ffprobe) { throw 'FFmpeg/ffprobe nisu pronađeni.' }
...
if (!$ytLine) { throw 'yt-dlp SHA256 zapis nije pronađen.' }
...
if (!$dMatch.Success) { throw 'Deno SHA256 zapis nije ispravan.' }
...
if (!$deno) { throw 'deno.exe nije pronađen.' }
Copy-Item $deno.FullName (Join-Path $DenoDir 'deno.exe') -Force

# ---------------------------------------------------------------------------
# SELF-CONTAINED Chromaprint/fpcalc.
# ---------------------------------------------------------------------------
...
if (!$fp) { throw 'fpcalc.exe nije pronađen.' }
...
# First-party Python compile and actual bundled-tool smoke checks.
...
if ($LASTEXITCODE -ne 0) { throw 'ffmpeg.exe smoke nije prošao.' }
& (Join-Path $FFmpegDir 'bin\ffprobe.exe') -version | Select-Object -First 1
if ($LASTEXITCODE -ne 0) { throw 'ffprobe.exe smoke nije prošao.' }
& $YtExe --version
if ($LASTEXITCODE -ne 0) { throw 'yt-dlp.exe smoke nije prošao.' }
& (Join-Path $DenoDir 'deno.exe') --version | Select-Object -First 1
if ($LASTEXITCODE -ne 0) { throw 'deno.exe smoke nije prošao.' }
& (Join-Path $FpDir 'fpcalc.exe') -version
if ($LASTEXITCODE -ne 0) { throw 'fpcalc.exe smoke nije prošao.' }

# Ensure package is genuinely self-contained before publishing.
$Required = @(
...
foreach ($rel in $Required) {
    if (!(Test-Path (Join-Path $Package $rel))) { throw "Nedostaje obavezna komponenta: $rel" }
}
...
if ($ZipSize -lt 120MB) { throw "Windows paket je neocekivano mali ($ZipSize bajta). Build nije kompletan." }
